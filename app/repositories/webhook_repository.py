...
class WebhookRepository(
    ...
    async def create_webhook_event(
        self, webhook_data: WebhookEventCreate, signature_verified: bool = False
    ) -> WebhookEvent:
        """
    ...
    async def get_pending_webhooks(
        self, limit: int = 100, older_than_minutes: Optional[int] = None
    ) -> List[WebhookEvent]:
        """
        ...
        return await self.get_multi_by_filters(
            *filters, limit=limit, order_by=WebhookEvent.received_at
        )
    ...
    async def get_failed_webhooks_for_retry(
        ...
        return await self.get_multi_by_filters(
            *filters, limit=limit, order_by=WebhookEvent.next_retry_at
        )

    async def get_webhooks_by_status(
        self, status: WebhookStatus, limit: int = 100, offset: int = 0
    ...
    async def get_webhooks_by_kyc_check(self, kyc_check_id: str) -> List[WebhookEvent]:
        """
    ...
    async def get_by_user_id(self, user_id: UUID) -> List[WebhookEvent]:
        """
        Get all webhook events for a specific user.

        Args:
            user_id: User UUID

        Returns:
            List of webhook events for the user
        """
        return await self.get_multi_by_filters(
            WebhookEvent.related_user_id == str(user_id),
            order_by=WebhookEvent.received_at,
        )
    ...
    async def increment_retry_count(
        self, webhook_id: UUID, next_retry_at: Optional[datetime] = None
    ) -> Optional[WebhookEvent]:
        """
    ...
    async def get_webhook_statistics(
        ...
        status_counts = await self.db.execute(
            query.with_entities(
                WebhookEvent.status, func.count(WebhookEvent.id).label("count")
        ...
        avg_processing_time = await self.db.execute(
            query.filter(WebhookEvent.processed_at.isnot(None)).with_entities(
                func.avg(
                    func.extract(
                        "epoch", WebhookEvent.processed_at - WebhookEvent.received_at
                    )
                ).label("avg_seconds")
            )
        )
