...
class WebhookEventBase(BaseModel):
    ...
    provider: str = Field(
        ..., min_length=1, max_length=100, description="Webhook provider name"
    )
    event_type: WebhookEventType = Field(..., description="Type of webhook event")
    provider_event_id: Optional[str] = Field(
        None, max_length=255, description="Provider's unique event identifier"
    )
...
class WebhookEventCreate(WebhookEventBase):
    ...
    related_kyc_check_id: Optional[str] = Field(
        None, description="Related KYC check ID"
    )
...
class WebhookEventUpdate(BaseModel):
    ...
    signature_verified: Optional[bool] = Field(
        None, description="Signature verification status"
    )
...
class WebhookEventResponse(WebhookEventBase):
    ...
    processed_at: Optional[datetime] = Field(
        None, description="When webhook was processed"
    )
    failed_at: Optional[datetime] = Field(
        None, description="When webhook processing failed"
    )
...
class WebhookProcessingResult(BaseModel):
    ...
    actions_taken: List[str] = Field(
        default=[], description="Actions taken during processing"
    )
...
class WebhookStatsResponse(BaseModel):
    ...
    provider_stats: Dict[str, Dict[str, int]] = Field(
        default={}, description="Per-provider statistics"
    )
    event_type_stats: Dict[str, Dict[str, int]] = Field(
        default={}, description="Per-event-type statistics"
    )
