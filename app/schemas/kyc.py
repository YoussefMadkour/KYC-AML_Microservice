...
class DocumentBase(BaseModel):
    ...
    document_number: Optional[str] = Field(
        None, description="Document number (will be encrypted)"
    )
...
class KYCCheckBase(BaseModel):
    ...
    provider: str = Field(
        ..., min_length=1, max_length=100, description="KYC provider name"
    )
...
class KYCCheckCreate(KYCCheckBase):
    ...
    documents: List[DocumentCreate] = Field(
        ..., min_items=1, description="Documents for verification"
    )
...
class KYCCheckResponse(KYCCheckBase):
    ...
    documents: List[DocumentResponse] = Field(
        default=[], description="Associated documents"
    )
    created_at: str = Field(..., description="Creation timestamp")
    updated_at: str = Field(..., description="Last update timestamp")
...
class KYCStatusUpdate(BaseModel):
    ...
    rejection_reason: Optional[str] = Field(
        None, description="Rejection reason if applicable"
    )
