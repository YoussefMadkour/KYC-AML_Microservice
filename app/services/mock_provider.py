...
class MockProviderInterface(ABC):
    ...
    async def submit_verification(
        self, documents: List[Dict], user_data: Dict, webhook_url: Optional[str] = None
    ) -> ProviderResponse:
        """
...
class BaseMockProvider(MockProviderInterface):
    ...
    async def submit_verification(
        ...
        processing_delay = random.uniform(
            self.min_processing_delay, self.max_processing_delay
        )
        ...
        if any(
            ...
            in [DocumentType.PASSPORT, DocumentType.DRIVER_LICENSE]
            for doc in documents
        ):
        ...
        response = ProviderResponse(
            ...
            raw_response=self._generate_raw_response(
                provider_reference, overall_status
            ),
        )
    ...
    def _generate_document_result(self, document: Dict) -> DocumentVerificationResult:
        ...
        status = random.choices(
            list(outcome_weights.keys()), weights=list(outcome_weights.values())
        )[0]
    ...
    def _get_document_outcome_weights(
        self, doc_type: DocumentType
    ) -> Dict[VerificationOutcome, float]:
        ...
        base_weights = {
            ...
            VerificationOutcome.REJECTED: 1.0
            - self.success_rate
    ...
    def _generate_document_issues(
        self, doc_type: DocumentType, status: VerificationOutcome
    ) -> List[str]:
    ...
    def _generate_extracted_data(self, doc_type: DocumentType, document: Dict) -> Dict:
        ...
        if doc_type == DocumentType.PASSPORT:
            ...
            base_data.update(
                {"license_class": "C", "restrictions": "NONE", "endorsements": "NONE"}
            )
        elif doc_type == DocumentType.UTILITY_BILL:
            base_data.update(
                {
    ...
    def _determine_overall_outcome(
        ...
        # Count outcomes
        approved_docs = sum(
            1 for doc in document_results if doc.status == VerificationOutcome.APPROVED
        )
        rejected_docs = sum(
            1 for doc in document_results if doc.status == VerificationOutcome.REJECTED
        )
        review_docs = sum(
            1
            for doc in document_results
            if doc.status == VerificationOutcome.MANUAL_REVIEW
        )

        total_docs = len(document_results)
        avg_confidence = (
            sum(doc.confidence_score for doc in document_results) / total_docs
            if total_docs > 0
            else 0.5
        )
    ...
    def _generate_raw_response(
        self, provider_reference: str, status: VerificationOutcome
    ) -> Dict:
...
class MockProviderFactory:
    ...
    @classmethod
    def create_provider(
        self, provider_type: Union[ProviderType, str], **kwargs
...
class MockProviderService:
    ...
    async def get_verification_result(
        self, provider_type: Union[ProviderType, str], provider_reference: str
    ) -> Optional[ProviderResponse]:
        """
    ...
    def configure_provider(
        self, provider_type: Union[ProviderType, str], **config
    ) -> None:
        """
            ...
            if hasattr(provider, "_verification_results"):
                ...
                if total > 0:
                    approved = sum(
                        ...
                        if r.overall_status == VerificationOutcome.APPROVED
                    )
                    rejected = sum(
                        1
                        for r in results.values()
                        if r.overall_status == VerificationOutcome.REJECTED
                    )
                    review = sum(
                        1
                        for r in results.values()
                        if r.overall_status == VerificationOutcome.MANUAL_REVIEW
                    )
                    ...
                    stats[provider_type.value] = {
                        ...
                        "avg_processing_time_ms": sum(
                            r.processing_time_ms for r in results.values()
                        )
                        / total,
                    }
