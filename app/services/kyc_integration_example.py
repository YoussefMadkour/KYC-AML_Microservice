...
class KYCIntegrationService:
    ...
    async def process_kyc_with_mock_provider(
        ...
        """
        logger.info(f"Processing KYC check {kyc_check_id} with mock provider {provider_type}")
        ...
        if kyc_check.status != KYCStatus.PENDING:
            logger.warning(f"KYC check {kyc_check_id} is not in pending status: {kyc_check.status}")
            return kyc_check
        ...
        try:
            ...
            status_update = KYCStatusUpdate(
                ...
                notes="Processing with mock provider"
            )
            ...
            if not updated_check:
                logger.error(f"Failed to update KYC check {kyc_check_id} to in_progress")
                return None
            ...
            provider_response = await self.mock_provider_service.submit_kyc_verification(
            ...
            final_status = self._map_provider_status_to_kyc_status(provider_response.overall_status)
            ...
            update_data = KYCCheckUpdate(
                ...
                verification_result={
                    ...
                    } if provider_response.biometric_result else None,
            ...
            logger.error(f"Error processing KYC check {kyc_check_id} with mock provider: {str(e)}")
