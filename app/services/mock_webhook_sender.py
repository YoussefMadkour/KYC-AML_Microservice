...
class WebhookPayloadTemplate(BaseModel):
    ...
    weight: float = Field(
        default=1.0, description="Probability weight for this outcome"
    )
...
class MockWebhookSender:
                ...
                WebhookPayloadTemplate(
                    ...
                    template_data={
                        ...
                        "result": {
                            ...
                            "score": 92,
                            "reasons": [
                                "High confidence document verification",
    ...
    async def schedule_webhook(
        ...
        webhook_provider = provider_mapping.get(
            provider_type, WebhookProvider.MOCK_PROVIDER_1
        )
        ...
        selected_template = random.choices(
            provider_templates, weights=[t.weight for t in provider_templates]
        )[0]
        ...
        target_url = (
            webhook_url
            or f"{self.config.base_webhook_url}/kyc/{webhook_provider.value}"
        )
    ...
    async def send_webhook_immediately(
        ...
        return WebhookDeliveryResult(
            ...
            or f"{self.config.base_webhook_url}/kyc/{provider_type.value}",
        )
    ...
    async def _send_webhook_after_delay(self, schedule_id: str, delay: float) -> None:
        ...
        try:
            ...
            webhook_data["status"] = (
                "completed" if delivery_result.success else "failed"
            )
    ...
    async def _deliver_webhook(self, webhook_data: Dict) -> WebhookDeliveryResult:
        ...
        signature = generate_webhook_signature(
            payload_json, webhook_provider, timestamp, self.config.signature_secret
        )
            ...
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    response = await client.post(
                        target_url, content=payload_json, headers=headers
                    )
                    ...
                    if response.status_code in [200, 201, 202]:
                        ...
                        logger.warning(
                            f"Webhook delivery failed: {error_msg} (attempt {attempt})"
                        )
    ...
    def _build_webhook_payload(
        self, webhook_data: Dict, template: WebhookPayloadTemplate
    ) -> Dict:
        """
        ...
        if webhook_provider == WebhookProvider.MOCK_PROVIDER_1:
            ...
            payload.update(
                {"version": "2.0", "source": "kyc_service", "test_mode": True}
            )

        return payload

    def _build_webhook_headers(
        self, provider: WebhookProvider, signature: str, timestamp: int
    ) -> Dict[str, str]:
        """
    ...
    def get_delivery_statistics(self) -> Dict[str, Any]:
        ...
        avg_delivery_time = (
            sum(d.delivery_time_ms for d in self._delivery_history) / total
        )
    ...
    def _get_provider_statistics(self) -> Dict[str, Dict[str, Any]]:
        ...
        for provider, stats in provider_stats.items():
            if stats["total"] > 0:
                provider_deliveries = [
                    d for d in self._delivery_history if f"/{provider}" in d.webhook_url
                ]
