...
class WebhookService:
    ...
    async def process_webhook_sync(
        ...
        try:
            ...
            try:
                ...
                await self.webhook_repo.update_webhook_status(
                    webhook_event.id, WebhookStatus.FAILED, error_message=error_msg
                )
    ...
    async def retry_webhook(
        self, webhook_id: UUID, force_retry: bool = False
    ) -> Tuple[bool, str]:
        """
        ...
        if not force_retry and not webhook.can_retry:
            return (
                False,
                f"Webhook cannot be retried (retry count: {webhook.retry_count}/{webhook.max_retries})",
            )
        ...
        retry_failed_webhook.apply_async(args=[str(webhook_id)], eta=next_retry_at)
    ...
    def _extract_related_ids(
        self, payload: str, event_type: WebhookEventType
    ...
    async def _process_kyc_status_update(
        self, webhook_event: WebhookEvent, payload: Dict[str, Any]
    ) -> Dict[str, List[str]]:
        """
        ...
        try:
            ...
            actions.append(
                f"Updated KYC status from {old_status} to {new_status.value}"
            )
