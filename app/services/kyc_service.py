...
class KYCService:
    ...
    def get_kyc_check(self, kyc_check_id: UUID, user_id: Optional[UUID] = None) -> Optional[KYCCheckResponse]:
        """
        Get KYC check by ID.
    ...
    def update_kyc_status(
        ...
        Raises:
            ...
            BusinessLogicError: If business rules are violated
        """
        logger.info(f"Updating KYC check {kyc_check_id} status to {status_update.status}")
        ...
        try:
            ...
            if updated_check:
                ...
                self._log_status_change(
                    ...
                    status_update.notes or f"Status updated by {updated_by or 'system'}",
                    updated_by
                )
    ...
    def _get_active_check(self, user_id: UUID) -> Optional[KYCCheck]:
        ...
        """
        active_statuses = [KYCStatus.PENDING, KYCStatus.IN_PROGRESS, KYCStatus.MANUAL_REVIEW]
    ...
    def _validate_documents(self, documents: List[DocumentCreate]) -> None:
        ...
        identity_types = {DocumentType.PASSPORT, DocumentType.DRIVER_LICENSE, DocumentType.NATIONAL_ID}
        if not document_types.intersection(identity_types):
            raise ValidationError("At least one identity document (passport, driver license, or national ID) is required")
    ...
    def _validate_document(self, document: DocumentCreate) -> None:
        ...
        if len(document.file_hash) != 64:
            raise ValidationError("File hash must be a valid SHA-256 hash (64 characters)")
        
        # Validate expiry date for documents that should have one
        expiry_required_types = {DocumentType.PASSPORT, DocumentType.DRIVER_LICENSE, DocumentType.NATIONAL_ID}
        if document.document_type in expiry_required_types and not document.expiry_date:
            raise ValidationError(f"{document.document_type.value} must have an expiry date")
    ...
    def _create_document(self, kyc_check_id: UUID, doc_data: DocumentCreate) -> Document:
        """
    ...
    def _to_response(self, kyc_check: KYCCheck) -> KYCCheckResponse:
        ...
        data = {
            ...
            "expires_at": kyc_check.expires_at.isoformat() if kyc_check.expires_at else None,
