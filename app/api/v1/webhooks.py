...
async def receive_kyc_webhook(
    ...
    try:
        ...
        logger.error(f"Error processing KYC webhook from {provider}: {e}", exc_info=True)
        raise HTTPException(
...
async def receive_aml_webhook(
    ...
    try:
        ...
        logger.error(f"Error processing AML webhook from {provider}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error processing webhook"
        )
...
async def simulate_kyc_webhook(
    ...
    Args:
        ...
        outcome: Verification outcome (approved, rejected, manual_review, pending, error)
    ...
    try:
        if immediate:
            ...
            task_result = send_immediate_webhook.apply_async(
                args=[kyc_check_id, user_id, provider_type, provider_reference, outcome],
...
async def get_webhook_simulation_stats(
    ...
    try:
        ...
        for webhook in scheduled_webhooks:
            ...
            scheduled_summary["by_status"][status] = scheduled_summary["by_status"].get(status, 0) + 1
