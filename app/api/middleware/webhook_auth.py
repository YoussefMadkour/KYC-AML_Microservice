...
class WebhookAuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if not provider:
            logger.warning(
                f"Could not extract provider from webhook path: {request.url.path}"
            )
            raise HTTPException(
        ...
        try:
            is_valid, verification_details = (
                webhook_signature_verifier.verify_webhook_request(
                    payload=payload,
                    headers=headers,
                    provider=provider,
            ...
            if not is_valid:
                error_message = verification_details.get(
            ...
            logger.info(
                f"Webhook signature verified successfully for provider: {provider}"
            )
...
class WebhookAuthDependency:
    ...
    async def __call__(self, request: Request, provider: str) -> Dict[str, any]:
        """
...
def get_webhook_auth(
    require_timestamp_validation: bool = True, log_verification_details: bool = True
