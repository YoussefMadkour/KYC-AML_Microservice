...
from sqlalchemy import Boolean, Column, DateTime, Enum as SQLEnum, Integer, JSON, String, Text
...
class WebhookEvent(BaseModel):
    ...
    provider = Column(
        ...
        nullable=False,
        index=True,
        doc="Name of the webhook provider"
    )
    ...
    provider_event_id = Column(
        ...
        nullable=True,
        index=True,
        doc="Provider's unique event identifier"
    )
    ...
    headers = Column(
        JSON,
        nullable=True,
        doc="HTTP headers from the webhook request"
    )
    
    raw_payload = Column(
        Text,
        nullable=False,
        doc="Raw webhook payload as received"
    )
    
    parsed_payload = Column(
        JSON,
        nullable=True,
        doc="Parsed and validated webhook payload"
    )
    ...
    processed_at = Column(
        DateTime,
        nullable=True,
        doc="When the webhook was successfully processed"
    )
    
    failed_at = Column(
        DateTime,
        nullable=True,
        doc="When the webhook processing failed"
    )
    ...
    related_user_id = Column(
        ...
        nullable=True,
        index=True,
        doc="ID of related user if applicable"
    )
    ...
    processing_notes = Column(
        Text,
        nullable=True,
        doc="Notes from webhook processing"
    )
    
    def __repr__(self) -> str:
        """String representation of the webhook event."""
        return f"<WebhookEvent(id={self.id}, provider={self.provider}, type={self.event_type}, status={self.status})>"
