...
class KYCCheck(BaseModel):
    ...
    provider = Column(
        String(100), nullable=False, doc="KYC provider name (e.g., 'mock_provider_1')"
    )

    provider_reference = Column(
        String(255), nullable=True, index=True, doc="External provider's reference ID"
    )

    # Verification details
    verification_result = Column(
        JSON, nullable=True, doc="Detailed verification results from provider"
    )

    risk_score = Column(
        String(20), nullable=True, doc="Risk assessment score (low, medium, high)"
    )
    ...
    completed_at = Column(
        DateTime, nullable=True, doc="When the KYC check was completed"
    )

    expires_at = Column(
        DateTime, nullable=True, doc="When the KYC verification expires"
    )

    # Additional information
    notes = Column(Text, nullable=True, doc="Additional notes or comments")

    rejection_reason = Column(
        String(500), nullable=True, doc="Reason for rejection if status is rejected"
    )

    # Relationships
    user = relationship(
        "User", back_populates="kyc_checks", doc="User associated with this KYC check"
    )
...
class Document(BaseModel):
    ...
    document_type = Column(
        SQLEnum(DocumentType), nullable=False, doc="Type of document"
    )

    # File information
    file_path = Column(
        String(500), nullable=False, doc="Path to the stored document file"
    )

    file_name = Column(String(255), nullable=False, doc="Original filename")

    file_size = Column(String(20), nullable=True, doc="File size in bytes")

    file_hash = Column(
        String(64), nullable=False, doc="SHA-256 hash of the file for integrity"
    )
    ...
    issue_date = Column(DateTime, nullable=True, doc="Document issue date")

    expiry_date = Column(DateTime, nullable=True, doc="Document expiry date")
    ...
    verification_notes = Column(
        Text, nullable=True, doc="Notes from document verification"
    )
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f"kyc_check_id={self.kyc_check_id})>"
        )
