...
class TaskMonitor:
    ...
    def get_kyc_processing_status(self, kyc_check_id: UUID) -> Dict[str, Any]:
        ...
        if kyc_check.status == KYCStatus.IN_PROGRESS and kyc_check.submitted_at:
            ...
            if elapsed < estimated_total:
                ...
                status_info["estimated_completion_seconds"] = int(
        ...
        if kyc_check.verification_result:
            result_summary = self._extract_verification_summary(
                kyc_check.verification_result
            )
    ...
    def get_task_history_for_kyc(
        self, kyc_check_id: UUID, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
            ...
            if active:
                for worker, tasks in active.items():
                    ...
                    stats["workers"].append(
                        {"name": worker, "active_tasks": len(tasks)}
                    )
                    ...
                    for task in tasks:
                        ...
                        stats["task_types"][task_name] = (
                            stats["task_types"].get(task_name, 0) + 1
                        )
                        ...
                        for queue in queues:
                            ...
                            if queue_name not in stats["queues"]:
                                stats["queues"][queue_name] = {
                                    ...
                                    "exchange": (
                                        queue.get("exchange", {}).get("name")
                                        if queue.get("exchange")
                                        else None
                                    ),
                                }
    ...
    def cancel_kyc_processing(
        self, kyc_check_id: UUID, reason: str = "Cancelled by user"
    ) -> bool:
        """
        ...
        try:
            ...
            if cancelled_count > 0:
                try:
                    kyc_check = self.kyc_repository.get(kyc_check_id)
                    if kyc_check and kyc_check.status in [
                        KYCStatus.PENDING,
    ...
    def _extract_verification_summary(
        ...
        try:
            ...
            if "processing_time_ms" in verification_result:
                summary["processing_time_ms"] = verification_result[
                    "processing_time_ms"
                ]
            ...
            if "document_results" in verification_result:
                ...
                summary["documents_approved"] = sum(
                    1 for doc in doc_results if doc.get("status") == "approved"
                )

            # Include biometric result if available
            if (
                "biometric_result" in verification_result
                and verification_result["biometric_result"]
            ):
...
class TaskLogger:
    ...
    def info(self, message: str, **kwargs):
        ...
        self.logger.info(
            message,
            extra={"task_name": self.task_name, "task_id": self.task_id, **kwargs},
        )
