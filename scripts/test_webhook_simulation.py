...
async def test_webhook_sender():
    ...
    if provider_stats:
        ...
        for provider, provider_stats in provider_stats.items():
            ...
            print(f"    Avg time: {provider_stats.get('avg_delivery_time_ms', 0):.0f}ms")
    else:
...
def test_payload_generation():
    ...
    for provider_type, outcome in test_combinations:
        ...
        webhook_data = {
            ...
            "webhook_provider": WebhookProvider.MOCK_PROVIDER_1.value if provider_type in [ProviderType.JUMIO, ProviderType.VERIFF] else WebhookProvider.MOCK_PROVIDER_2.value
        }
        ...
        if templates:
            ...
            signature = webhook_sender.config.signature_secret and verify_webhook_signature(
                payload_json,
...
async def test_webhook_scheduling():
    ...
    if stats['recent_deliveries']:
        ...
        for delivery in stats['recent_deliveries'][-3:]:  # Show last 3
            print(f"    Success: {delivery['success']}, Time: {delivery['delivery_time_ms']}ms")
