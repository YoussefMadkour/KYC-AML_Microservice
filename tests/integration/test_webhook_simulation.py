...
from app.tasks.webhook_tasks import send_immediate_webhook, simulate_provider_webhook
...
class TestMockWebhookSender:
    ...
    async def test_schedule_webhook_rejected(self, webhook_sender, sample_kyc_data):
        ...
        webhook_data = next(
            w for w in scheduled_webhooks if w["schedule_id"] == schedule_id
        )
    ...
    @pytest.mark.asyncio
    async def test_schedule_webhook_manual_review(
        self, webhook_sender, sample_kyc_data
    ):
        ...
        assert (
            "review_reasons" in template.template_data["result"]
            or "review_notes" in template.template_data["result"]
        )
    ...
    async def test_webhook_payload_generation(self, webhook_sender, sample_kyc_data):
        ...
        for provider_type in providers:
            ...
            assert (
                payload["provider_reference"] == sample_kyc_data["provider_reference"]
            )
    ...
    async def test_webhook_signature_generation(self, webhook_sender, sample_kyc_data):
        ...
        headers = webhook_sender._build_webhook_headers(
            webhook_provider, signature, timestamp
        )
...
class TestWebhookSimulationTasks:
    ...
    def test_send_immediate_webhook_task(self, mock_sender, sample_task_data):
        ...
        mock_sender.send_webhook_immediately = AsyncMock(
            return_value=mock_delivery_result
        )
...
class TestWebhookSimulationAPI:
    ...
    @pytest.mark.asyncio
    async def test_simulate_kyc_webhook_endpoint(
        ...
        with patch("app.api.v1.webhooks.get_current_admin_user") as mock_admin:
            mock_admin.return_value = type(
                "User", (), {"id": uuid4(), "is_admin": True}
            )()

            with patch("app.tasks.webhook_tasks.send_immediate_webhook") as mock_task:
                mock_task.apply_async.return_value = type(
                    "Result", (), {"id": "task_123"}
                )()
    ...
    @pytest.mark.asyncio
    async def test_simulate_kyc_webhook_scheduled(
        self, async_client: AsyncClient, admin_headers
    ):
        ...
        with patch("app.api.v1.webhooks.get_current_admin_user") as mock_admin:
            ...
            with patch(
                "app.tasks.webhook_tasks.simulate_provider_webhook"
            ) as mock_task:
                mock_task.apply_async.return_value = type(
                    "Result", (), {"id": "task_456"}
                )()
    ...
    @pytest.mark.asyncio
    async def test_invalid_provider_type_api(
        self, async_client: AsyncClient, admin_headers
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_simulation_stats_api(
        self, async_client: AsyncClient, admin_headers
    ):
        ...
        with patch("app.api.v1.webhooks.get_current_admin_user") as mock_admin:
            ...
            with patch(
                "app.services.mock_webhook_sender.mock_webhook_sender"
            ) as mock_sender:
                ...
                response = await async_client.get(
                    "/api/v1/webhooks/simulate/stats", headers=admin_headers
                )
    ...
    @pytest.mark.asyncio
    async def test_clear_simulation_history_api(
        self, async_client: AsyncClient, admin_headers
    ):
        ...
        with patch("app.api.v1.webhooks.get_current_admin_user") as mock_admin:
            ...
            with patch(
                ...
                response = await async_client.post(
                    "/api/v1/webhooks/simulate/clear", headers=admin_headers
                )
    ...
    @pytest.mark.asyncio
    async def test_list_scheduled_webhooks_api(
        self, async_client: AsyncClient, admin_headers
    ):
        ...
        with patch("app.api.v1.webhooks.get_current_admin_user") as mock_admin:
            ...
            with patch(
                ...
                response = await async_client.get(
                    "/api/v1/webhooks/simulate/scheduled", headers=admin_headers
                )
