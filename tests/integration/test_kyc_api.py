...
class TestKYCCheckCreation:
    ...
    def test_create_kyc_check_multiple_documents(
    ...
    def test_create_kyc_check_missing_documents(
        self, client, test_db, create_test_user
    ):
        """Test KYC check creation with missing documents."""
        headers = get_auth_headers(client, "testuser@example.com", "TestPassword123")

        kyc_data = {"provider": "mock_provider", "documents": []}

        response = client.post("/api/v1/kyc/checks", json=kyc_data, headers=headers)

        assert response.status_code == 422

    def test_create_kyc_check_invalid_document_hash(
        self, client, test_db, create_test_user
    ):
    ...
    def test_create_kyc_check_expired_document(self, client, test_db, create_test_user):
        ...
        kyc_data = get_sample_kyc_data()
        kyc_data["documents"][0]["expiry_date"] = (
    ...
    def test_create_kyc_check_duplicate_active(
        self, client, test_db, create_test_user, create_kyc_check
    ):
...
class TestKYCCheckRetrieval:
    ...
    def test_get_kyc_check_success(
        self, client, test_db, create_test_user, create_kyc_check
    ):
    ...
    def test_get_kyc_check_admin_access(
        self, client, test_db, create_admin_user, create_test_user, create_kyc_check
    ):
    ...
    def test_get_kyc_check_compliance_access(
        self,
        client,
        test_db,
        ...
        headers = get_auth_headers(
            client, "compliance@example.com", "CompliancePassword123"
        )
        check_id = str(create_kyc_check.id)
    ...
    def test_get_kyc_check_other_user_forbidden(
        self, client, test_db, create_test_user, create_second_user, create_kyc_check
    ):
...
class TestKYCCheckListing:
    ...
    def test_list_kyc_checks_success(
        self, client, test_db, create_test_user, create_kyc_check
    ):
    ...
    def test_list_kyc_checks_with_status_filter(
        self, client, test_db, create_test_user, create_kyc_check
    ):
    ...
    def test_list_kyc_checks_admin_with_user_filter(
        self, client, test_db, create_admin_user, create_test_user, create_kyc_check
    ):
    ...
    def test_list_kyc_checks_user_filter_forbidden(
        self, client, test_db, create_test_user, create_second_user, create_kyc_check
    ):
        ...
        assert response.status_code == 403
        assert (
            "Not enough permissions to filter by user ID" in response.json()["detail"]
        )
...
class TestKYCCheckUpdates:
    ...
    def test_update_kyc_check_admin_success(
        self, client, test_db, create_admin_user, create_test_user, create_kyc_check
    ):
        ...
        response = client.put(
            f"/api/v1/kyc/checks/{check_id}", json=update_data, headers=headers
        )
    ...
    def test_update_kyc_check_invalid_status_transition(
    ...
    def test_update_kyc_check_not_found(self, client, test_db, create_admin_user):
        ...
        response = client.put(
            f"/api/v1/kyc/checks/{fake_check_id}", json=update_data, headers=headers
        )

        assert response.status_code == 404
        assert "KYC check not found" in response.json()["detail"]

    def test_update_kyc_check_non_admin_forbidden(
        self, client, test_db, create_test_user, create_kyc_check
    ):
    ...
    def test_update_kyc_status_compliance_success(
        self,
        client,
        test_db,
        create_compliance_user,
        ...
        response = client.patch(
            f"/api/v1/kyc/checks/{check_id}/status", json=status_update, headers=headers
        )
    ...
    def test_update_kyc_status_rejected_with_reason(
        self,
        client,
        test_db,
        create_compliance_user,
    ...
    def test_update_kyc_status_rejected_without_reason(
        self,
        client,
        test_db,
        create_compliance_user,
    ...
    def test_update_kyc_status_non_compliance_forbidden(
        self, client, test_db, create_test_user, create_kyc_check
    ):
...
class TestKYCCheckHistory:
    ...
    def test_get_kyc_history_success(
        self,
        client,
        test_db,
        create_compliance_user,
    ...
    def test_get_kyc_history_not_found(self, client, test_db, create_compliance_user):
        ...
        response = client.get(
            f"/api/v1/kyc/checks/{fake_check_id}/history", headers=headers
        )

        assert response.status_code == 404
        assert "KYC check not found" in response.json()["detail"]

    def test_get_kyc_history_non_compliance_forbidden(
        self, client, test_db, create_test_user, create_kyc_check
    ):
...
class TestKYCStatistics:
    ...
    def test_get_kyc_statistics_success(
        self,
        client,
        test_db,
        create_compliance_user,
    ...
    def test_get_kyc_statistics_non_compliance_forbidden(
        self, client, test_db, create_test_user
    ):
...
class TestKYCPendingChecks:
    ...
    def test_get_pending_checks_success(
        self,
        client,
        test_db,
        create_compliance_user,
    ...
    def test_get_pending_checks_with_limit(
        self,
        client,
        test_db,
        create_compliance_user,
    ...
    def test_get_pending_checks_non_compliance_forbidden(
        self, client, test_db, create_test_user
    ):
...
class TestKYCWorkflows:
    ...
    def test_complete_kyc_verification_workflow(
        self, client, test_db, create_test_user, create_compliance_user
    ):
        """Test complete KYC verification workflow."""
        user_headers = get_auth_headers(
            client, "testuser@example.com", "TestPassword123"
        )
        compliance_headers = get_auth_headers(
            client, "compliance@example.com", "CompliancePassword123"
        )
        ...
        response = client.post(
            "/api/v1/kyc/checks", json=kyc_data, headers=user_headers
        )
        assert response.status_code == 201
        ...
        response = client.patch(
            f"/api/v1/kyc/checks/{check_id}/status",
            json=status_update,
    ...
    def test_kyc_rejection_workflow(
        self, client, test_db, create_test_user, create_compliance_user
    ):
        ...
        assert "Document quality insufficient" in str(
            final_data.get("rejection_reason", "")
        )

    def test_kyc_access_control_workflow(
        self,
        client,
        test_db,
        create_test_user,
        ...
        kyc_data = get_sample_kyc_data()
        response = client.post(
            "/api/v1/kyc/checks", json=kyc_data, headers=user1_headers
        )
        assert response.status_code == 201
        check_id = response.json()["id"]
        ...
        response = client.get(
            f"/api/v1/kyc/checks/{check_id}", headers=compliance_headers
        )
        assert response.status_code == 200
        ...
        response = client.put(
            f"/api/v1/kyc/checks/{check_id}", json=update_data, headers=admin_headers
        )
        assert response.status_code == 200
        ...
        response = client.put(
            f"/api/v1/kyc/checks/{check_id}", json=update_data, headers=user1_headers
        )
        assert response.status_code == 403
