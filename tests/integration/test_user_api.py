...
class TestUserProfileEndpoints:
    ...
    def test_update_user_profile_invalid_country(self, client, test_db, create_test_user):
...
class TestUserListEndpoints:
    ...
    def test_list_users_admin_success(self, client, test_db, create_admin_user, create_test_user, create_second_user):
    ...
    def test_list_users_with_pagination(self, client, test_db, create_admin_user, create_test_user, create_second_user):
    ...
    def test_list_users_with_role_filter(self, client, test_db, create_admin_user, create_test_user):
    ...
    def test_list_users_with_active_filter(self, client, test_db, create_admin_user, create_test_user):
...
class TestUserDetailEndpoints:
    ...
    def test_get_user_by_id_admin_access(self, client, test_db, create_admin_user, create_test_user):
    ...
    def test_get_user_by_id_forbidden_other_user(self, client, test_db, create_test_user, create_second_user):
...
class TestUserAdminEndpoints:
    ...
    def test_update_user_admin_success(self, client, test_db, create_admin_user, create_test_user):
    ...
    def test_update_user_admin_duplicate_email(self, client, test_db, create_admin_user, create_test_user, create_second_user):
        """Test admin updating user with duplicate email."""
        headers = get_auth_headers(client, "admin@example.com", "AdminPassword123")
        user_id = str(create_test_user.id)
        
        update_data = {
            "email": "user2@example.com"  # Email of second user
        }
    ...
    def test_update_user_admin_not_found(self, client, test_db, create_admin_user):
        ...
        response = client.put(f"/api/v1/users/{fake_user_id}", json=update_data, headers=headers)
        
        assert response.status_code == 404
        assert "User not found" in response.json()["detail"]
    
    def test_update_user_non_admin_forbidden(self, client, test_db, create_test_user, create_second_user):
    ...
    def test_deactivate_user_admin_success(self, client, test_db, create_admin_user, create_test_user):
    ...
    def test_activate_user_admin_success(self, client, test_db, create_admin_user, create_test_user):
    ...
    def test_deactivate_user_not_found(self, client, test_db, create_admin_user):
        ...
        response = client.post(f"/api/v1/users/{fake_user_id}/deactivate", headers=headers)
    ...
    def test_activate_user_not_found(self, client, test_db, create_admin_user):
        ...
        response = client.post(f"/api/v1/users/{fake_user_id}/activate", headers=headers)
        
        assert response.status_code == 404
        assert "User not found" in response.json()["detail"]
    
    def test_deactivate_user_non_admin_forbidden(self, client, test_db, create_test_user, create_second_user):
        """Test non-admin cannot deactivate users."""
        headers = get_auth_headers(client, "testuser@example.com", "TestPassword123")
        other_user_id = str(create_second_user.id)
        
        response = client.post(f"/api/v1/users/{other_user_id}/deactivate", headers=headers)
        
        assert response.status_code == 403
    
    def test_activate_user_non_admin_forbidden(self, client, test_db, create_test_user, create_second_user):
        """Test non-admin cannot activate users."""
        headers = get_auth_headers(client, "testuser@example.com", "TestPassword123")
        other_user_id = str(create_second_user.id)
        
        response = client.post(f"/api/v1/users/{other_user_id}/activate", headers=headers)
...
class TestUserManagementWorkflows:
    ...
    def test_admin_user_management_workflow(self, client, test_db, create_admin_user, create_test_user):
        ...
        # 5. Reactivate user
        response = client.post(f"/api/v1/users/{user_id}/activate", headers=admin_headers)
        assert response.status_code == 200
        reactivated_user = response.json()
        assert reactivated_user["is_active"] is True
    
    def test_user_access_control_workflow(self, client, test_db, create_test_user, create_second_user, create_admin_user):
