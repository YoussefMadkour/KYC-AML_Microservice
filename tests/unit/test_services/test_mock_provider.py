...
class TestBaseMockProvider:
    ...
    def test_generate_confidence_score(self, provider):
        ...
        approved_score = provider._generate_confidence_score(
            VerificationOutcome.APPROVED
        )
        rejected_score = provider._generate_confidence_score(
            VerificationOutcome.REJECTED
        )
        review_score = provider._generate_confidence_score(
    ...
    def test_generate_document_issues(self, provider):
        ...
        approved_issues = provider._generate_document_issues(
            DocumentType.PASSPORT, VerificationOutcome.APPROVED
        )
        rejected_issues = provider._generate_document_issues(
            DocumentType.PASSPORT, VerificationOutcome.REJECTED
        )
        review_issues = provider._generate_document_issues(
            DocumentType.PASSPORT, VerificationOutcome.MANUAL_REVIEW
        )
    ...
    def test_generate_extracted_data(self, provider):
        ...
        extracted = provider._generate_extracted_data(
            DocumentType.PASSPORT, passport_doc
        )
    ...
    def test_determine_overall_outcome_approved(self, provider):
        ...
        status, risk, confidence = provider._determine_overall_outcome(
            doc_results, biometric_result
        )
    ...
    async def test_submit_verification(
        self, provider, sample_documents, sample_user_data
    ):
        ...
        assert (
            response.biometric_result is not None
        )  # Should have biometric for passport
    ...
    async def test_get_verification_result(
        ...
        response = await provider.submit_verification(
            sample_documents, sample_user_data
        )
...
class TestSpecificProviders:
    ...
    def test_jumio_provider(self):
        ...
        raw_response = provider._generate_raw_response(
            "JUM_123", VerificationOutcome.APPROVED
        )
    ...
    def test_onfido_provider(self):
        ...
        raw_response = provider._generate_raw_response(
            "ONF_123", VerificationOutcome.APPROVED
        )
    ...
    def test_veriff_provider(self):
        ...
        raw_response = provider._generate_raw_response(
            "VER_123", VerificationOutcome.MANUAL_REVIEW
        )
...
class TestMockProviderFactory:
    ...
    def test_create_provider_with_config(self):
        ...
        provider = MockProviderFactory.create_provider(
            ProviderType.JUMIO, success_rate=0.9, manual_review_rate=0.05
        )
...
class TestMockProviderService:
    ...
    def sample_user_data(self):
        """Sample user data for testing."""
        return {"user_id": "user_123", "first_name": "John", "last_name": "Doe"}
    ...
    @pytest.mark.asyncio
    async def test_submit_kyc_verification(
        self, service, sample_documents, sample_user_data
    ):
    ...
    def test_configure_provider(self, service):
        ...
        service.configure_provider(
            provider_type=ProviderType.JUMIO, success_rate=0.95, manual_review_rate=0.03
        )
