...
class TestAuthService:
    ...
    def test_authenticate_user_success(self, auth_service, mock_user_repo, sample_user):
        ...
        mock_user_repo.authenticate.assert_called_once_with("test@example.com", "TestPassword123")
        mock_security.create_token_pair.assert_called_once_with("test-user-123")
    ...
    def test_authenticate_user_inactive_account(self, auth_service, mock_user_repo, sample_user):
    ...
    def test_refresh_token_success(self, auth_service, mock_user_repo, sample_user):
        ...
        mock_security.get_subject_from_token.assert_called_once_with(refresh_token, "refresh")
    ...
    def test_change_password_success(self, auth_service, mock_user_repo, sample_user):
        ...
        with patch('app.services.auth_service.SecurityUtils') as mock_security:
            ...
            result = auth_service.change_password(sample_user, current_password, new_password)
        
        # Verify
        assert result is True
        
        mock_security.verify_password.assert_called_once_with(current_password, sample_user.hashed_password)
        mock_security.get_password_hash.assert_called_once_with(new_password)
        mock_user_repo.update.assert_called_once_with(sample_user, {"hashed_password": "new_hashed_password"})
    ...
    def test_change_password_incorrect_current_password(self, auth_service, sample_user):
        ...
        with patch('app.services.auth_service.SecurityUtils') as mock_security:
            ...
            with pytest.raises(HTTPException) as exc_info:
                auth_service.change_password(sample_user, current_password, new_password)
