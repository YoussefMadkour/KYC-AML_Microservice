...
from app.services.mock_provider import ProviderResponse, ProviderType, VerificationOutcome, RiskLevel
from app.models.kyc import KYCStatus, DocumentType
from app.schemas.kyc import KYCCheckResponse, DocumentResponse
...
class TestKYCIntegrationService:
    ...
    def test_map_provider_status_to_kyc_status(self, integration_service):
        ...
        assert integration_service._map_provider_status_to_kyc_status("approved") == KYCStatus.APPROVED
        assert integration_service._map_provider_status_to_kyc_status("rejected") == KYCStatus.REJECTED
        assert integration_service._map_provider_status_to_kyc_status("manual_review") == KYCStatus.MANUAL_REVIEW
        assert integration_service._map_provider_status_to_kyc_status("pending") == KYCStatus.IN_PROGRESS
        assert integration_service._map_provider_status_to_kyc_status("error") == KYCStatus.REJECTED
        assert integration_service._map_provider_status_to_kyc_status("unknown") == KYCStatus.MANUAL_REVIEW
    ...
    async def test_process_kyc_with_mock_provider_not_found(self, integration_service):
        ...
        integration_service.kyc_service.get_kyc_check.assert_called_once_with(kyc_check_id)
    
    @pytest.mark.asyncio
    async def test_process_kyc_with_mock_provider_wrong_status(self, integration_service, sample_kyc_check):
    ...
    async def test_process_kyc_with_mock_provider_success(
        ...
        # Setup mocks
        integration_service.kyc_service.get_kyc_check.return_value = sample_kyc_check
        integration_service.kyc_service.update_kyc_status.return_value = sample_kyc_check
        ...
        with patch.object(
            ...
            result = await integration_service.process_kyc_with_mock_provider(kyc_check_id, "jumio")
    ...
    @pytest.mark.asyncio
    async def test_process_kyc_with_mock_provider_error(self, integration_service, sample_kyc_check):
