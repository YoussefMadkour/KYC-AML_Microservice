...
class TestWebhookRepository:
    ...
    async def test_get_webhooks_by_status(self, webhook_repo):
        ...
        result = await webhook_repo.get_webhooks_by_status(
    ...
    async def test_get_webhooks_by_kyc_check(self, webhook_repo):
        ...
        kyc_check_id = "kyc123"
        mock_webhooks = [
            WebhookEvent(id=uuid4(), related_kyc_check_id=kyc_check_id)
        ]
    ...
    async def test_get_webhooks_by_user(self, webhook_repo):
        ...
        result = await webhook_repo.get_webhooks_by_user(
            user_id, limit=30, offset=5
        )
    ...
    async def test_increment_retry_count(self, webhook_repo):
        ...
        result = await webhook_repo.increment_retry_count(
            webhook_id, next_retry_at
        )
    ...
    async def test_cleanup_old_webhooks(self, webhook_repo):
        ...
        result = await webhook_repo.cleanup_old_webhooks(
            days_old=30, keep_failed=True
        )
